    ...
    <table class="table-style-0">
      ...
      <tbody>
        <tr
          ...
          :id="`${table_title_to_id(table.title)}_${r}`"
          :class="{
            active: shownName == `${table_title_to_id(table.title)}_${r}`
          }"
        >
      ...
      for (let i = 0; i < connections.length; i++) {
        ...
        if (elStart != null && elEnd != null) {
          const myLine = new LeaderLine(elStart, elEnd, {
            ...
            endLabel: LeaderLine.captionLabel(connection.Label, {
              offset: [25, -25]
            }),
            hide: true
          })
...
function processContext(context: any) {
  ...
  return new Data(context.content.tables, context.content.connections)
}
</script>
